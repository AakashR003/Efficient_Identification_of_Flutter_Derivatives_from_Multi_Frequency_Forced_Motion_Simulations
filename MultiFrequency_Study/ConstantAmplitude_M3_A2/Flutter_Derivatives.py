...
def plot_amplitude_vs_frequency(components, path_DFT):
    ...
    for frequency_tuple in frequency_list:
        frequency, h_amplitude, tita_amplitude, _ = frequency_tuple
        U_reduced_local_plot_amplitude_vs_frequency.append(round(wind_velocity / (frequency * B), 2))

    plt.title(f'Spectrum - {parts[2]} - {parts[-1][4:-4]} - Reduced_Velocities = {U_reduced_local_plot_amplitude_vs_frequency} - Combination_case: {parts[0][0:1]}')
...
for freq in frequency:
    ...
    for prescribed_motion in prescribed_motions:
        ...
        for frequency_list in freq:
            ...
            path_DFT_Fy = os.path.join(path_results, "DFT_Fy_" + "combination_case_" + str(str(len(frequency_list))) + ".png")
            path_DFT_moment = os.path.join(path_results, "DFT_moment_" + "combination_case_" + str(str(len(frequency_list))) + ".png")
            ...
            os.path.exists(path_Amplitude_Phase) and os.remove(path_Amplitude_Phase)
            os.path.exists(path_flutter_derivatives) and os.remove(path_flutter_derivatives)
            for frequency_tuple in frequency_list:
                ...
                if prescribed_motion == "Heave":
                    ...
                    with open(path_Amplitude_Phase, "a") as f:    
