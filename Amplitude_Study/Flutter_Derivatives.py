...
for freq in frequency:
    ...
    for prescribed_motion in prescribed_motions:
        ...
        for frequency_list in freq:
            ...
            current_path = str(iteration_variable2) + path_base
            current_path = os.path.join(current_path, str(len(frequency_list)) + "_" +str(iteration_variable2), prescribed_motion)
            ...
            path_force_components = os.path.join(path_results, "Fy_and_Components" + ".png")
            path_moment_components = os.path.join(path_results, "Mx_and_Components"  + ".png")
            path_motion_components = os.path.join(path_results, "Motion_and_Components"  + ".png")
            path_DFT_Fy = os.path.join(path_results, "DFT_Fy_" + "amplitude_case_" + str(iteration_variable1) + ".png")
            path_DFT_moment = os.path.join(path_results, "DFT_moment_" + "amplitude_case_" + str(iteration_variable1) + ".png")
            ...
            #FFT
            # Compute FFT once for each signal with maximum needed components
            ...
            # Plot using pre-computed components (sliced to needed length)
            plot_signal_and_components(fy, force_components_full[:len(frequency_list)], sample_frequency, path_force_components)
            plot_signal_and_components(moment, moment_components_full[:len(frequency_list)], sample_frequency, path_moment_components)

            # Extract dictionaries from top 10
            ...
            # Use full component list for DFT plots
            plot_amplitude_vs_frequency(force_components_full, path_DFT_Fy)
            plot_amplitude_vs_frequency(moment_components_full, path_DFT_moment)
            ...
            for frequency_tuple in frequency_list:
                ...
                path_fitted_signal = os.path.join(path_results, "Fitted_Signal" + str(frequency) + ".png")
                path_flutter_derivatives = os.path.join(path_results,"Flutter_Derivatives" + ".txt")   
                path_Amplitude_Phase = os.path.join(path_results,"Amplitude_and_Phase" + ".txt")
                ...
                if prescribed_motion == "Heave":
                    ...
                    with open(path_Amplitude_Phase, "w") as f:    
                        f.write("Amplitude and Phase Data of Signals\n")
                        f.write("===================================\n")
                        f.write(f"Frequency        : {frequency}\n")
                        f.write(f"Reduced Velocity : {U_reduced}\n")
                        f.write(f"Heave Amplitude  : {h_amplitude}\n")
                        f.write(f"Fy Amplitude     : {F}\n")
                        f.write(f"Phase Fy         : {phase_diff_Fy}\n")
                        f.write(f"Moment Amplitude : {M}\n")
                        f.write(f"Phase Mx         : {phase_diff_Mx}\n")   
                    ...
                    with open(path_Amplitude_Phase, "w") as f:    
                        ...
                        f.write(f"Frequency        : {frequency}\n")
                        f.write(f"Reduced Velocity : {U_reduced}\n")
                        f.write(f"Tita Amplitude   : {tita_amplitude}\n")
                        f.write(f"Fy Amplitude     : {F}\n")
                        f.write(f"Phase Fy         : {phase_diff_Fy}\n")
                        f.write(f"Moment Amplitude : {M}\n")
                        f.write(f"Phase Mx         : {phase_diff_Mx}\n")
