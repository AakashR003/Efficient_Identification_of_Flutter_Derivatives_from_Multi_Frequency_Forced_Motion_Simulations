...
output_nonlinearity = os.path.join(base_path, "Combined_Results", "Normalised_Nonlinearity_Study")

for motion in motions:
    motion_path = os.path.join(output_nonlinearity, motion)
    os.makedirs(motion_path, exist_ok=True)

    # Amplitude_Case (x-axis)
    Amplitude_Case = np.array([0, 1, 2, 3, 4, 5, 6]) 
    ...
    # Get U_reduced values from the first combination (they should be the same across all Amplitude_Case)
    u_reduced_values = [results[motion][0][row_idx]["U_reduced"] for row_idx in range(n_rows)]
    
    for row_idx in range(n_rows):
        # Extract data for this row across all Amplitude_Case
    ...
    make_plot(
        Amplitude_Case, fy_normalized_rows,
        "Amplitude_Case",
        "Normalized Fy Amplitude (Fy/Amplitude)",
        f"{motion}: Normalized Fy vs Amplitude_Case",
        os.path.join(motion_path, "NormalizedFy_vs_Amplitude_Case.png"),
        #phase_window=0.2,
        labels=row_labels
    )

    make_plot(
        Amplitude_Case, phase_fy_rows,
        "Amplitude_Case",
        "Phase Fy [rad]",
        f"{motion}: Phase Fy vs Amplitude_Case",
        os.path.join(motion_path, "PhaseFy_vs_Amplitude_Case.png"),
        #phase_window=0.5,
        labels=row_labels
    )

    make_plot(
        Amplitude_Case, m_normalized_rows,
        "Amplitude_Case",
        "Normalized Moment Amplitude (M/Amplitude)",
        f"{motion}: Normalized Moment vs Amplitude_Case",
        os.path.join(motion_path, "NormalizedMoment_vs_Amplitude_Case.png"),
        #phase_window=0.02,
        labels=row_labels
    )

    make_plot(
        Amplitude_Case, phase_mx_rows,
        "Amplitude_Case",
        "Phase Mx [rad]",
        f"{motion}: Phase Mx vs Amplitude_Case",
        os.path.join(motion_path, "PhaseMx_vs_Amplitude_Case.png"),
        #phase_window=0.5,
        labels=row_labels
    )
    ...
    make_plot(
        Amplitude_Case, fy_normalized_to_first,
        "Amplitude_Case",
        "Normalized Fy (relative to Combination 1)",
        f"{motion}: Normalized Fy (First Comb = 1)",
        os.path.join(motion_path, "NormalizedFy_vs_Amplitude_Case_RelativeToFirst.png"),
        labels=row_labels
    )

    make_plot(
        Amplitude_Case, phase_fy_normalized_to_first,
        "Amplitude_Case",
        "Phase Fy (relative to Combination 1)",
        f"{motion}: Phase Fy (First Comb = 1)",
        os.path.join(motion_path, "PhaseFy_vs_Amplitude_Case_RelativeToFirst.png"),
        labels=row_labels
    )

    make_plot(
        Amplitude_Case, m_normalized_to_first,
        "Amplitude_Case",
        "Normalized Moment (relative to Combination 1)",
        f"{motion}: Normalized Moment (First Comb = 1)",
        os.path.join(motion_path, "NormalizedMoment_vs_Amplitude_Case_RelativeToFirst.png"),
        labels=row_labels
    )

    make_plot(
        Amplitude_Case, phase_mx_normalized_to_first,
        "Amplitude_Case",
        "Phase Mx (relative to Combination 1)",
        f"{motion}: Phase Mx (First Comb = 1)",
        os.path.join(motion_path, "PhaseMx_vs_Amplitude_Case_RelativeToFirst.png"),
        labels=row_labels
    )
