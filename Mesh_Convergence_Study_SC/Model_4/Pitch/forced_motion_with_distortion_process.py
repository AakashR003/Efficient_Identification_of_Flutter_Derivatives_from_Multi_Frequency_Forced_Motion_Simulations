...
class ForcedMotionWithDistortionProcess(KratosMultiphysics.Process):
    ...
    def __init__(self, Model, params):
        ...
        self.model_part_names["1"] = params["model_part_name_1"].GetString()
        self.model_parts["1"] = Model[self.model_part_names["1"]]
        self.reference_points["1"] = params["reference_point_1"].GetVector()
        ...
        if (self.write_output_files):
            ...
            box = "1"
            self.output_files[box] = {}
                
            # Only write output files if this is rank 0
            if (self.model_parts[box].GetCommunicator().MyPID() == 0):
                ...
                for case in ['motion', 'forces']:
                    ...
                    case_file_handler_params["file_name"].SetString(output_file_name + '_box' + box + '_' + case + '.dat')

                    # Create file
                    file_header = self._GetFileHeader(box, case)
                    self.output_files[box][case] = TimeBasedAsciiFileWriterUtility(
                        self.model_parts[box],
    ...
    def ExecuteInitializeSolutionStep(self):
        ...
        MeshMovingApplication.MoveModelPart(
            self.model_parts[box],
            [0, 0, 1],                                        # rotation axis
            self.motion["theta"+box],                         # rotation angle
            self.reference_points[box],                       # one point of the rotation axis
            [self.motion["v"+box], self.motion["w"+box], 0])  # translation (after the rotation)
    ...
    def _UpdateSolutionStepIncrement(self):
        ...
        # Get current time
        time = self.model_parts["1"].ProcessInfo[KratosMultiphysics.TIME]
        ...
        box = "1"
        self.current_reference_points[box] = [
            self.motion["v"+box],
            self.motion["w"+box],
            0
        ]
    ...
    def ExecuteFinalizeSolutionStep(self):
        ...
        if self.write_output_files:
            ...
            force, moment = self._EvaluateGlobalForces(box)
            ...
            if (self.model_parts[box].GetCommunicator().MyPID() == 0):
                ...
                motion_output = [self.motion["v"+box], self.motion["w"+box], self.motion["theta"+box]]
                ...
                self.output_files[box]['forces'].write(' '.join(force_output) + '\n')
                self.output_files[box]['motion'].write(' '.join(motion_output) + '\n')


    def _EvaluateGlobalForces(self, box):
    ...
    def _GetFileHeader(self, box, case):
        # Generates the headers of the ascii file depending on which output it has

        if case == "motion":
            header = '# Motion for model part ' + self.model_part_names[box] + '\n'
            header += '# Time v w theta\n'
        elif case == "forces":
            header = '# Forces for model part ' + self.model_part_names[box] + '\n'
